...
sequenceDiagram
    ...
    Note over SH: warn if OPENROUTER_API_KEY missing
...
| 5 | `start.ts` warns if `OPENROUTER_API_KEY` is not set — non-fatal, service continues |
| 6 | `start.ts` calls `GET /webhooks` on the Hostfully API to validate the API key. If the response is 401 or 403, startup **exits immediately** with a clear error message. Retries up to 3 times on transient failures (5xx, network errors) |
...
What goes into the AI and what comes back out on every message. All calls go through OpenRouter (`openrouter.ai/api/v1/chat/completions`) using the model configured in `OPENROUTER_MODEL`.
