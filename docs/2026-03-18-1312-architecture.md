...
graph LR
    ...
    class IDX event
    class HC,KB,TT,SLK,WR,HDL service
    class DD storage
    class OC external
```
...
| 4 | `slackApp` — creates the Bolt app and registers approve, reject, and edit action handlers (wired with `hostfullyClient` and `threadTracker`) |
| 5 | `webhookReceiver` — starts the HTTP server on port 3001; `dedup` is created as a module-level singleton inside `webhook-receiver.ts` |
...
graph LR
    ...
    class WR,PL,HC,TT,SB,KB,SC,HDL service
    class AL storage
```
...
| 2 | `pipeline` calls `hostfullyClient` to GET message content, thread participants, lead details, and property name |
...
| 7 | `slackApp handlers` call `hostfullyClient.sendMessage()` on approve or edit to POST to Hostfully |
| 8 | `slackApp handlers` call `threadTracker.clear()` after every action to reset thread state |
| 9 | `slackApp handlers` import `slack-blocks` directly to build updated Slack messages |
| 10 | `slackApp handlers` call `appendAuditLog()` (inline in `handlers.ts`, using Node `fs`) to write to `logs/actions.jsonl` |
...
flowchart TD
    ...
    subgraph Ingestion["Phase 1 · Ingestion"]
        W([Webhook POST arrives on port 3001])
        D{Already processed?}
        SKIP([Skip silently])
    ...
    subgraph Posting["Phase 4 · Posting"]
        ...
        TRK -->|"no"| NEW
        REP -->|"13. store ts"| TRACK
        NEW -->|"13. store ts"| TRACK
        TRACK --> WAIT
    end

    MARK -->|"3. fetch"| FM
    KB -->|"9. classify + draft"| CL
    BK -->|"11. check thread"| TRK
    ...
    class D,FT,TRK decision
    class FM,CL,BK,KB,REP,NEW service
    class MARK,TRACK storage
    class TH,FL,FP external
```
...
flowchart LR
    ...
    class MSG,APR,RJR,ESR event
    class ACT decision
    class APM,APH,RJM,EDM,MOD,EDS,EDH service
    class APC,AP_AUD,RJC,RJ_AUD,EDC,ED_AUD storage
    class AP_ERR,ED_ERR error
```
