...
| 2 | `kbReader` — searches `knowledge-base/common.md` + the property-specific KB file for context relevant to each guest message |
...
sequenceDiagram
    ...
    Note over SH: start proxy health monitor (background, CLAUDE_MODE=proxy only)
    Note over SH: warn if fallback enabled but ANTHROPIC_API_KEY missing
    SH->>HF: GET /webhooks (validateApiKey)
    HF-->>SH: 200 OK — key valid
...
| 7 | A background health monitor starts polling the proxy port every 30 seconds (only when `CLAUDE_MODE=proxy`). It logs `⚠️ Claude proxy is DOWN` and `✅ Claude proxy recovered` on state transitions — no Slack alerts, log-only |
| 8 | If `CLAUDE_FALLBACK_TO_API` is enabled (default) but `ANTHROPIC_API_KEY` is not set, a warning is logged — non-fatal, service continues |
| 9 | `start.ts` calls `GET /webhooks` on the Hostfully API to validate the API key. If the response is 401 or 403, startup **exits immediately** with a clear error message. Retries up to 3 times on transient failures (5xx, network errors) |
| 10 | Bun starts the main application process |
| 11 | `.env` is loaded and validated — missing required vars exit immediately with a clear error |
| 12 | Core skills are instantiated: Hostfully client, KB reader, thread tracker |
| 13 | The Slack Bolt app is created and the three action handlers (approve, reject, edit) are registered |
| 14 | Bolt connects to Slack via Socket Mode (outbound WebSocket, no inbound port needed) |
| 15 | The pipeline context is created, wiring all skills together into a single orchestrator |
| 16 | The webhook receiver starts listening on port 48901 for Hostfully POST requests |
