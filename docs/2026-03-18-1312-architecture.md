...
How Papi Chulo v2 works, broken down into focused diagrams. Each diagram has numbered steps with explanations below it. v2 adds OpenClaw as a persistent agent runtime running alongside the Bun service.

---

## 1. System Context

The external systems v2 talks to and the order in which they interact during a single guest message. The key difference from v1: OpenClaw runs as a background daemon, providing agent identity, persistent memory, and a future expansion path for proactive tasks.
...
The skill-based architecture of v2. `src/index.ts` creates and wires all skills together. Each skill is a focused module in its own directory under `src/skills/`.
...
How OpenClaw fits into the v2 architecture. It runs as a persistent background daemon, separate from the Bun process, and provides the agent identity, memory, and future expansion path.
